...
dnf install --installroot ${scratchmnt} bash coreutils glibc-minimal-langpack microdnf \
	--disablerepo="*" \
	--enablerepo=BaseOS \
	--releasever 8 \
	--nodocs \
...
buildah config --label name=el8-microdnf ${newcontainer}
buildah config --cmd /bin/bash ${newcontainer}

# commit the image
buildah unmount ${newcontainer}
buildah commit ${newcontainer} el8-microdnf
